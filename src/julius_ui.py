...
def draw_icon(surf, app, x, y, size=60):
    ...
    if name == "Terminal":
        ...
        for ang in range(0,360,90):
            r2  = math.radians(ang)
            sx2 = cx2+int(math.cos(r2)*7)
            sy2 = cy2+int(math.sin(r2)*7)
            ex  = cx2+int(math.cos(r2)*13)
            ey  = cy2+int(math.sin(r2)*13)
        ...
        pygame.draw.lines(surf, ac, False,
            [(mid-6,t+4),(mid+6,cy2),(mid-6,b-4),
             (mid-6,t+4),(mid,t),(mid+6,cy2),(mid,b),(mid-6,b-4)], 2)
        ...
        t2=font_small.render("SSH",True,ac)
        surf.blit(t2,(cx2-t2.get_width()//2,cy2-t2.get_height()//2))
        ...
        for r2 in range(3):
            for c3 in range(3):
                pygame.draw.circle(surf,ac,(x+14+c3*8,cy2+2+r2*7),2)
        ...
        pygame.draw.arc(surf,ac,(cx2-2,cy2-9,16,14),
            math.pi*0.8,math.pi*2,2)
        ...
        for rad,op in [(12,50),(8,120),(4,200)]:
            pygame.draw.circle(surf,(*ac,op),(cx2,cy2),rad,1)
        ...
        pygame.draw.lines(surf,ac,False,[(cx2-6,y+13),(cx2,y+8),(cx2+6,y+13)],2)
        ...
        for ang in range(0,360,45):
            r2=math.radians(ang)
            x1=cx2+int(math.cos(r2)*8)
            y1=cy2+int(math.sin(r2)*8)
            x2=cx2+int(math.cos(r2)*12)
            y2=cy2+int(math.sin(r2)*12)
...
def draw_status_bar(dark=True):
    txt_color = WHITE if dark else (30,30,40)
    now = datetime.datetime.now()
    t   = now.strftime("%H:%M")
    ts  = font_status.render(t, True, txt_color)
    screen.blit(ts, (16, 12))
    bat_x = W-52
    pygame.draw.rect(screen, txt_color, (bat_x,12,28,13), 1, border_radius=3)
    pygame.draw.rect(screen, txt_color, (bat_x+28,15,3,7), border_radius=1)
    pygame.draw.rect(screen, GREEN,     (bat_x+2,14,22,9), border_radius=2)
    for i in range(3):
        bx = W-92+i*7
        bh = 5+i*2
        pygame.draw.rect(screen, (*txt_color,80+i*60),(bx,18-bh//2,5,bh),border_radius=1)
...
def draw_lock_screen():
    ...
    screen.blit(ts, (W//2-ts.get_width()//2, H//2-60))
    screen.blit(ds, (W//2-ds.get_width()//2, H//2+30))

    if swipe_count == 0:
        hint = font_hint.render("swipe left to unlock", True, (*WHITE,120))
    elif swipe_count == 1:
        hint = font_hint.render("swipe left again", True, (*WHITE,180))
    else:
    ...
    for i in range(3):
        sx = 20 + i*8
        sy = H//2 + 80
        pygame.draw.circle(screen, (*WHITE,80+i*40 if i<swipe_count else 40),(sx,sy),3)
...
def draw_home_screen():
    ...
    dot_y = H-78
    for i in range(2):
        col2 = WHITE if i==current_page else (*WHITE,70)
        pygame.draw.circle(screen,col2,(W//2-6+i*12,dot_y),
    ...
    for section in SETTINGS_SECTIONS:
        ...
        for item in section["items"]:
            if 36 < y < H-60:
                ...
                if item["type"] == "toggle":
                    col3 = GREEN if val else (80,80,90)
                    rr(screen,col3,(W-58,y+8,40,20),10)
...
def handle_settings_tap(pos, items_flat):
    for entry in items_flat:
        iy  = entry["y"]
        itm = entry["item"]
        if iy < pos[1] < iy+40:
            key  = itm["key"]
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONUP:
            ...
            is_swipe_left  = dx < -50 and abs(dy)<50
            is_swipe_right = dx >  50 and abs(dy)<50
            is_swipe_down  = dy >  70 and abs(dx)<50
            is_swipe_up    = dy < -50 and abs(dx)<50
            ...
            if state == STATE_LOCK:
                ...
                if is_swipe_left and current_page == 0:
                    current_page = 1
                ...
                if is_swipe_right or is_swipe_down:
                    ...
                    settings_scroll = min(settings_scroll+40, 400)
                elif is_tap:
                    handle_settings_tap(event.pos, items_flat_cache)
                if is_tap and event.pos[1] > H-70:
                    state = STATE_HOME
